...
- (void)stackCardView:(CardView *)cardView
{
    if (![self.stackOfCardViews containsObject:cardView]) {
        ...
        cardView.frame = [self.cardsContainerView convertRect:cardView.frame toView:self.stackView];
...
- (void)dismissStackOfCards
{
    ...
    for (CardView *cardView in self.stackOfCardViews) {
        cardView.frame = [self.stackView convertRect:cardView.frame toView:self.cardsContainerView];
